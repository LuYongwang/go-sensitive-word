...
func main() {
	...
	// 动态自定义敏感词白名单
	err = filter.DelWord("武汉海鲜市场", "武汉")
	if err != nil {
		log.Fatalf("删除敏感词发生了错误, err:%v", err)
		return
	}

	// 等待异步处理完成（DFA/AC 通过 channel 异步处理，需要短暂延迟）
	time.Sleep(100 * time.Millisecond)

	sensitiveText := "李世民和老祖秦始皇的是忘年交，他们两个相约一起去武汉海鲜市场玩耍. 并且晚点要一起做Ｈttp测试"
