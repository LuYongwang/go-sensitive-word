...
func TestIgnoreCase_FindOne(t *testing.T) {
	m, err := NewFilter(StoreOption{Type: StoreMemory}, FilterOption{Type: FilterDfa})
	if err != nil {
		t.Fatal(err)
	}
	if err := m.AddWord("fuck"); err != nil {
		t.Fatal(err)
	}
	text := "fuCK the bad words."
	got := m.FindOne(text)
	if got != "fuCK" {
		t.Fatalf("expect fuCK, got %q", got)
	}
}

func TestFullWidth_FindOne(t *testing.T) {
	m, err := NewFilter(StoreOption{Type: StoreMemory}, FilterOption{Type: FilterDfa})
	if err != nil {
		t.Fatal(err)
	}
	...
	got := m.FindOne(text)
	if got != "ｆｕｃｋ" {
		t.Fatalf("expect ｆｕｃｋ, got %q", got)
	}
}

func TestDigitNormalize(t *testing.T) {
	// 需要配置启用数字归一化，暂时跳过
	t.Skip("需要支持自定义配置")
}

func TestTradSimp(t *testing.T) {
	// 需要配置启用繁简归一，暂时跳过
	t.Skip("需要支持自定义配置")
}

func TestEnglishVariant(t *testing.T) {
	m, err := NewFilter(StoreOption{Type: StoreMemory}, FilterOption{Type: FilterDfa})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDFA(t *testing.T) {
	...
	// 动态自定义敏感词
	err = filter.AddWord("测试1", "测试2", "成小王")
	if err != nil {
		fmt.Println(err)
		return
	}
